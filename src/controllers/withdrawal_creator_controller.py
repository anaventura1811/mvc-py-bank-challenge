...
class WithdrawalCreatorController(TransactionCreatorControllerInterface):
    def __init__(self,
                 withdrawal_repository: TransactionInterface,
    ...
    def create(self, transaction: Dict) -> Dict:
        self.__validate_withdrawal(transaction)
        current_balance = self.__get_current_balance_in_db(transaction["account_id"])
        transaction_limit = self.__generate_withdrawal_limit(current_balance)
        self.__validate_transaction_limit(transaction, current_balance, transaction_limit)
        formatted_payment = self.__generate_withdrawal_from_dict(
            transaction, current_balance, transaction_limit)
        self.__insert_transaction_in_db(formatted_payment)
        response = self.__format_response(formatted_payment)
        return response

    def __validate_withdrawal(self, transaction: Dict) -> None:
    ...
    def __generate_withdrawal_limit(self, balance: float) -> float:
        now = datetime.now().hour()
        if now > 20 or now <= 6:
            return 1000.00
        return balance

    def __validate_transaction_limit(self,
                                     transaction: Dict, balance: float,
        ...
        if transaction["value"] > balance:
            raise Exception('Sem limite para esta transação')

    def __generate_withdrawal_from_dict(
            self, payload: Dict, current_balance: float, transaction_limit: float) -> Withdrawal:
        remaning_value = current_balance - payload["value"]
        transaction = {
            "account_id": payload["account_id"],
            "value": payload["value"],
            "date": datetime.now(),
            "balance_after_transaction": remaning_value if remaning_value > 0 else 0.0,
    ...
    def __insert_transaction_in_db(self, transaction: Withdrawal) -> None:
        self.__withdrawal_repository.create_transaction(transaction)

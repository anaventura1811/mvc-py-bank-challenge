from datetime import date
from src.models.sqlite.entities.transaction import Withdrawal
from .withdrawal_list_controller import WithdrawalListController


class MockWithdrawalRepository:
    def list_transactions(self, account_id: int):
        return [
            Withdrawal(
                date=date(2024, 11, 9).isoformat(),
                id=123,
                account_id=account_id,
                balance_after_transaction=1600,
                value=200,
                description="Retirada"),
            Withdrawal(
                date=date(2024, 11, 8).isoformat(),
                id=122,
                account_id=account_id,
                balance_after_transaction=1800,
                value=200,
                description="Retirada")]


def test_list_transactions():
    controller = WithdrawalListController(MockWithdrawalRepository())
    account_id = 456
    response = controller.list(account_id)

    formatted_transactions = [{"date": "2024-11-09", "type": "withdrawal",
                               "id": 123, "account_id": 456, "balance_after_transaction": 1600,
                               "value": 200, "description": "Retirada"},
                              {"date": "2024-11-08", "type": "withdrawal", "id": 122,
                               "account_id": 456,
                               "balance_after_transaction": 1800,
                               "value": 200, "description": "Retirada"}]
    expected_response = {
        "data": {
            "type": "Withdrawal",
            "count": 2,
            "attributes": formatted_transactions
        }
    }

    assert "data" in response
    assert response == expected_response
